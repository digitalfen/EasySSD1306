...
unsigned int Event::render(Adafruit_SSD1306 *display)
{
    if (preset == PROGRESS_BAR)
    {
        return presetEvent1(display);
    }
    else if (preset == CIRCLE_SWEEP)
    {
        return presetEvent2(display);
    }
    else if (preset == DOT_SEQUENCE)
    {
        return presetEvent3(display);
    }
    else
    {
        ...
        display->print("Unknown Event preset.");
        display->display();
        delay(2000);
...
unsigned int Event::presetEvent1(Adafruit_SSD1306 *display)
{
    display->clearDisplay();
    for (int i = 0; i <= 100; i++)
    {
...
unsigned int Event::presetEvent2(Adafruit_SSD1306 *display)
{
...
unsigned int Event::presetEvent3(Adafruit_SSD1306 *display)
{
    display->clearDisplay();
...
// Implementações de presetEvent2, presetEvent3, presetEvent4, presetEvent5
