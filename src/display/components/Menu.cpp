...
unsigned int Menu::render(Adafruit_SSD1306 *display)
{
    if (preset == CURSOR_LIST)
    {
        return presetMenu1(display);
    }
    else if (preset == HIGHLIGHT_LIST)
    {
        return presetMenu2(display);
    }
    else if (preset == VERTICAL_SELECTOR)
    {
        return presetMenu3(display);
    }
    else if (preset == HORIZONTAL_SELECTOR)
    {
        return presetMenu4(display);
    }
    ...
    else
    {
        ...
        display->print("Unknown menu preset.");
        display->display();
        delay(2000);
...
unsigned int Menu::presetMenu1(Adafruit_SSD1306 *display)
{
...
unsigned int Menu::presetMenu2(Adafruit_SSD1306 *display)
{
    if (DisplayManager::selectedItem < 0)
    {
...
unsigned int Menu::presetMenu3(Adafruit_SSD1306 *display)
{
    if (DisplayManager::selectedItem < 0)
    {
        DisplayManager::selectedItem = options.size() - 1; // Vai para o último item
    ...
    display->clearDisplay(); // Limpa a tela antes de desenhar o menu
...
unsigned int Menu::presetMenu4(Adafruit_SSD1306 *display)
{
    if (DisplayManager::selectedItem < 0)
    {
        DisplayManager::selectedItem = options.size() - 1; // Vai para o último item
